class MedicinesInventoriesController < ApplicationController
  before_action :load_medicine, only:[:show,:update,:destroy]  
  # def index
  ...
  def update
    MedicineInventory.where(id: params[:id]).update(availability:params[:availability],med_name:params[:med_name],qty:params[:qty])
