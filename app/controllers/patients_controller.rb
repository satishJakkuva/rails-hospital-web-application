class PatientsController < ApplicationController
  before_action :load_patient, only: %i[show update]
  def index
    @patients = Patient.all.select do |block|
      {
  ...
  def patient_medicines_by_doctor
    ...
    patient_medicines = @patients.flat_map do |patient|
      patient.medicines_inventories.only(:availability, :med_name, :qty)
    end
